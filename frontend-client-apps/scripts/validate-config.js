...
const SPEAKER_ENV_VARS = [
  ...
  { name: 'VITE_RUM_GUEST_ROLE_ARN', required: false },
  { name: 'VITE_RUM_IDENTITY_POOL_ID', required: false },
  { name: 'VITE_RUM_ENDPOINT', required: false },
];

const LISTENER_ENV_VARS = [
  {
    name: 'VITE_WEBSOCKET_URL',
    required: true,
    validator: (value) => {
...
function validateApp(appName, envVars) {
  const result = {
    app: appName,
    valid: true,
    errors: [],
